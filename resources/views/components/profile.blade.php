                        ...
                        <form action="{{ route('uploadImage') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <div class="image-buttons flex md:justify-start">
                                <button id="uploadButton" type="button"
                                    class="open-modal-button cursor-pointer rounded-lg border-2 border-gray-600 bg-none px-3 py-1 text-gray-600 hover:bg-gray-200 hover:text-gray-800 mr-2"
                                    onclick="toggleElementVisibility('imageUploaderModal')">Upload</button>
                                    <button id="deleteButton" type="button"
                                    class="cursor-pointer rounded-lg bg-gray-600 px-3 py-1 text-white transition-colors duration-500 ease-in-out hover:bg-red-700 @if (!$user->profile_image) opacity-50 cursor-not-allowed disabled:opacity-50 disabled:cursor-not-allowed @endif"
                                    onclick="toggleElementVisibility('deleteModal')" @if (!$user->profile_image) disabled @endif>Delete</button>                                
                            </div>
                        </form>
